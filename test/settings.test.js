...
describe('settings store', () => {
  ...
  it('STORAGE_KEYS 應包含所有必要的 key', async () => {
    ...
    expect(STORAGE_KEYS.POST_MESSAGE).toBe('grpc_debugger_postmessage');
    expect(STORAGE_KEYS.REFLECTION).toBe('grpc_debugger_reflection');
  ...
  it('enablePostMessage 預設為 true', async () => {
    vi.resetModules();
    localStorageMock.clear();
    const { enablePostMessage } = await import('../src/stores/settings.js');
    let value;
    enablePostMessage.subscribe(v => value = v)();
    expect(value).toBe(true);
  });

  it('enableReflection 預設為 true', async () => {
    vi.resetModules();
    localStorageMock.clear();
    const { enableReflection } = await import('../src/stores/settings.js');
    let value;
    enableReflection.subscribe(v => value = v)();
    expect(value).toBe(true);
  });
});
